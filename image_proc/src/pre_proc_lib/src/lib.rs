...
pub extern "C" fn create_proc_handle(img: *const RustImage) -> *mut c_void {
    ...
    let buf = unsafe { &*img };
    println!(
        "convert to slice, width: {}, height: {}, channels: {}",
    ...
    let src_img = match buf.channels {
        ...
        _ => {
            println!(
                "Unsupported number of channels: {}. Supported: 1 (Grayscale), 3 (RGB), 4 (RGBA)",
                buf.channels
            );
            return std::ptr::null_mut();
        }
    ...
    let pre_proc = PreProc::new(&src_img.to_owned());
...
pub extern "C" fn to_gray(handle: *mut c_void, buffer: *mut RustImage) {
    ...
    let gray_img_buffer = match ConvterColor::to_gray(&proc.image()) {
        Ok(img) => img,
        Err(e) => {
...
pub extern "C" fn free_threshold_buffer(buffer: *const RustImage) {
    ...
    let vec_data = unsafe { Vec::from_raw_parts(buf.buffer as *mut u8, buffer_size, buffer_size) };
    // vec_data在这里自动被释放
    drop(vec_data);
}
